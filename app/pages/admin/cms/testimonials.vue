<template>
  <UDashboardPanel id="testimonials" :ui="{ body: 'lg:py-12' }">
    <template #header>
      <UDashboardNavbar :title="$t('testimonials')">
        <template #leading>
          <UDashboardSidebarCollapse />
        </template>
        <template #right>
          <TestimonialsAddModal :refresh="refresh" />
        </template>
      </UDashboardNavbar>
    </template>

    <template #body>
      <UTable
        :data="testimonials?.data || []"
        :columns="columns"
        :loading="pending"
        class="w-full"
        :ui="{
          td: 'md:py-3 py-2',
          th: 'md:py-3.5 py-2',
        }"
      >
        <template #loading>
          <DataTableLoading />
        </template>
      </UTable>
    </template>
  </UDashboardPanel>
</template>

<script setup>
import {
  LazyDeleteModal,
  UDropdownMenu,
  UButton,
  LazyTestimonialsEditModal,
} from "#components";

const { t } = useI18n();
const overlay = useOverlay();

useHead({
  title: () => t("testimonials"),
});

const RESOURCE_PATH = "cms/testimonials";

const {
  data: testimonials,
  pending,
  refresh,
} = await useApiFetch(`${RESOURCE_PATH}`, { lazy: true });

const deleteModal = overlay.create(LazyDeleteModal);
const editTestimonialsModel = overlay.create(LazyTestimonialsEditModal);

const columns = [
  {
    id: "testimonial_name",
    accessorKey: "name",
    header: () => t("name"),
    cell: ({ row }) =>
      h("p", { class: "font-semibold text-highlighted" }, row.original.name),
  },
  {
    id: "testimonial_rating",
    accessorKey: "rating",
    header: () => t("rating"),
    cell: ({ row }) =>
      h("p", { class: "font-semibold text-highlighted" }, row.original.rating),
  },
  {
    id: "testimonial_message",
    accessorKey: "message",
    header: () => t("message"),
    cell: ({ row }) =>
      h("p", { class: "font-semibold text-highlighted" }, row.original.message),
  },
  {
    id: "testimonial_source",
    accessorKey: "source",
    header: () => t("source"),
    cell: ({ row }) =>
      h("p", { class: "font-semibold text-highlighted" }, row.original.source),
  },
  {
    id: "actions",
    accessorKey: "actions",
    header: "",
    cell: ({ row }) =>
      h(
        UDropdownMenu,
        {
          items: [
            [
              {
                label: t("delete"),
                icon: "i-lucide-trash",
                color: "error",
                onClick: () => {
                  deleteModal.open({
                    itemPath: `${RESOURCE_PATH}/${row.original.id}`,
                    refresh,
                  });
                },
              },
            ],
            [
              {
                label: t("edit"),
                icon: "i-lucide-edit-2",
                onClick: () => {
                  editTestimonialsModel.open({
                    refresh,
                    testimonial: row.original,
                  });
                },
              },
            ],
          ],
        },
        () =>
          h(UButton, {
            icon: "i-lucide-ellipsis-vertical",
            color: "neutral",
            variant: "ghost",
          }),
      ),
  },
];
</script>
